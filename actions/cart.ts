...
export const validateCoupon = async (code: string) => {
    ...
    const coupon = await db.query.Coupon.findFirst({
        ...
        where: (Coupon, { ilike }) => ilike(Coupon.code, code),
    });
    ...
    if (user.is_anonymous && coupon.usageLimit)
        return {
            code: 400,
            message: 'User must be logged in to apply this promo code!',
        } as ServerResponse;

    const currentDateTime = new Date(Date.now());

    if (
        coupon.validFrom <= currentDateTime &&
        (!coupon.validUntil ||
            (coupon.validUntil && coupon.validUntil >= currentDateTime))
    )
        return {
            code: 200,
